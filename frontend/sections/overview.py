...
kpi_row([
    {"label": "Total spend",  "value": fmt_money(kpis["total_spend"]),
     ...
     "help": "How much we paid for all AI tools this period (OpenAI + "
             "Anthropic + Cursor combined). The arrow shows change vs the "
             "previous equal period."},
    {"label": "Tokens in",    "value": fmt_int(kpis["tokens_in"]),
     "delta": kpis["tokens_in_delta"],
     "help": "Tokens we sent to the models (input). Roughly the volume of "
             "context, prompts and chat history."},
    {"label": "Tokens out",   "value": fmt_int(kpis["tokens_out"]),
     "delta": kpis["tokens_out_delta"],
     "help": "Tokens the models generated back (output). Output tokens are "
             "usually 3-5× more expensive than input."},
    {"label": "Active users", "value": str(kpis["active_users"]),
     "delta": kpis["active_users_delta"],
     "help": "Unique people who made at least 1 request this period. "
             "Reflects real reach of AI tooling across the team."},
])
if CFG.github_enabled:
    kpi_row([{
        "label": "AI code share", "value": fmt_pct(kpis["ai_code_share"]),
        "delta": kpis["ai_code_share_delta"],
        "help": "Share of new code lines generated by AI vs total team "
                "commits. Cursor flags ai_lines directly; bot commits "
                "count as 100% AI.",
    }])

section(
    "Spend over time",
    help="Daily spend per provider over the period. Sharp spikes usually "
         "mean a new heavy user, automation/cron, or an expensive "
         "reasoning model.",
)
