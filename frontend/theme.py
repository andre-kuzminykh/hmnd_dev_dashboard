...
/* --- Dark navy theme — matches the Genesys-style brand background ---
   Applied globally so the dashboard reads as a single dark canvas with
   navy gradient + soft blue glow, mirroring the logo background. */
[data-testid="stAppViewContainer"],
[data-testid="stAppViewContainer"] > .main,
.stApp {
    background:
        radial-gradient(circle at top left,  rgba(73,83,216,0.30), transparent 38%),
        radial-gradient(circle at top right, rgba(73,83,216,0.18), transparent 32%),
        linear-gradient(180deg, #0a1232 0%, #050816 100%) !important;
    color: #e6e9f5;
}
[data-testid="stHeader"] { background: transparent !important; }

/* Text colours on dark canvas */
.stApp h1, .stApp h2, .stApp h3, .stApp h4 { color: #f1f5fb !important; }
.stApp p, .stApp label, .stApp span, .stApp div {
    color: #e6e9f5;
}

/* KPI cards — same height regardless of how much content is inside.
   Background slightly lighter than canvas so they read as a "raised" panel. */
.hmnd-kpi {
    background: rgba(255,255,255,0.04) !important;
    border: 1px solid rgba(255,255,255,0.08) !important;
    backdrop-filter: blur(6px);
    box-shadow: 0 14px 32px -24px rgba(0,0,0,0.5) !important;
    min-height: 110px;             /* unify card height across rows */
    display: flex; flex-direction: column; justify-content: center;
}
.hmnd-kpi .label { color: #94a3b8 !important; }
.hmnd-kpi .value { color: #f8fafc !important; }
.hmnd-kpi .note  { color: #cbd5e1 !important; }

/* Section panels (Spend Breakdown, segment chart, etc.) — same dark surface */
.hmnd-hero, .hmnd-section {
...
.hmnd-table th, .hmnd-table td {
    background: transparent !important;
    color: #e6e9f5 !important;
    border-color: rgba(255,255,255,0.06) !important;
}
.hmnd-table thead tr { background: rgba(255,255,255,0.05) !important; }

/* Streamlit native widgets: keep their controls readable on dark BG */
[data-baseweb="select"] > div {
    background: rgba(255,255,255,0.04) !important;
    border-color: rgba(255,255,255,0.12) !important;
    color: #f1f5fb !important;
}
/* Every input / textarea / number field — force light text on dark surface.
   Without this, typed characters render in default near-black on our dark
   canvas and look invisible. */
input, textarea,
...
[data-testid="stMultiSelect"] [data-baseweb="tag"] {
    background: rgba(73,83,216,0.25) !important;
    color: #f1f5fb !important;
    border-color: rgba(73,83,216,0.5) !important;
}
[data-testid="stMultiSelect"] [data-baseweb="tag"] svg { fill: #f1f5fb !important; }
